...
public class WebArithmeticController { 
	...
	//Exposing the request mapping of '/multiply for the http://localhost:4444/multiply -> to be consumed by the multiplication form 
	...
	public String doMultiply(@RequestParam(defaultValue="1") String multiplicand, 
		...
		try { 
			...
			return "product"; // The name of the new Thymeleaf template for multiplication results 
		} catch (Exception e) { 
			logger.severe("An error occurred during multiplication: " + e.getMessage()); 
			return "error"; // Redirect to an error page if multiplication fails 
		} 
	} 
}