...
public class WebArithmeticController { 
	...
	public String doAdd(@RequestParam(defaultValue = "0") String addend1, 
		...
		logger.info("Received addends: " + addend1 + " and " + addend2); 
		// Call the addition service with the new input types 
		String sum = additionService.add(addend1, addend2);//Triggering a call to the Addition Service add method. 
		// Check if the response contains an error message 
		if (sum.contains("error")) { 
			model.addAttribute("error", sum); // Add error message to model 
			return "error"; // Return to error view 
		} 
		// Log the result 
		logger.info("Sum: " + sum); 
